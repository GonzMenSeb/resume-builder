...
class LegacyProfileExtractor:
    """Legacy extractor kept for backward compatibility - uses Claude CLI with JSON parsing."""
    ...
    def extract(self, raw_text: str) -> PersonProfile:
        """Extract using Claude CLI with JSON parsing."""
        if not raw_text.strip():
            raise ExtractionError("Cannot extract profile from empty text")
    ...
    def _call_claude(self, user_prompt: str) -> str:
        ...
        try:
            result = self._cli.invoke(prompt=user_prompt, system=PROFILE_EXTRACTION_SYSTEM)
        except ClaudeCLIError as e:
            logger.exception("Claude CLI invocation failed")
            raise ExtractionError(f"Claude CLI error: {e}") from e
    ...
    def _validate_profile(self, data: dict[str, object]) -> PersonProfile:
